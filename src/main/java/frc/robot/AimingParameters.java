...
public class AimingParameters {
    ...
    private static void calculate(Translation2d robotPos, ChassisSpeeds fieldRelativeSpeed, Translation3d aimPoint) {
        ...
        var predictedDistToAimPoint = Meters.of(aimPoint.toTranslation2d().getDistance(predictedRobotPos));
        ...
        AimingParameters.effectiveDistance.mut_replace(predictedDistToAimPoint);
        AimingParameters.pivotAltitude.mut_replace(AimingConstants.pivotAltitude.get(predictedDistToAimPoint));
        AimingParameters.targetShooterSpeed.mut_replace(AimingConstants.targetShooterSpeed.get(predictedDistToAimPoint));
        AimingParameters.minimumShooterSpeed.mut_replace(AimingConstants.minimumShooterSpeed.get(predictedDistToAimPoint));
